...
def image_stitching(frame, boxes):

   

    resolution_ratio_x = (640/576)*(int(frame.size[0])/640)

    resolution_ratio_y = (640/576)*(int(frame.size[1])/640)
    ...
    for box in boxes:
        ...
        section_image = frame.crop((int(box[1]*resolution_ratio_x), int(box[0]*resolution_ratio_y), int(box[3]*resolution_ratio_x), int(box[2]*resolution_ratio_y)))

       

        patches.append(section_image)

 

    total_width = sum(patch.width for patch in patches)

    max_height = max(patch.height for patch in patches)
    ...
    for patch in patches:
        ...
        output_image.paste(patch, (x_offset, 0))
...
def infer(frame, count=0):
    ...
    output_resize = transforms.Resize((640, 640))
    ...
    output_image = Image.new("RGB", (frame.size[0], frame.size[1]))
    ...
    output_image = image_stitching(frame, boxes)
    output_image = output_resize(output_image)
    #output_image.show()
    ...
    # plt.colorbar()
    # plt.savefig('./cropped_1/heatmap_'+str(count)+".png")
    # plt.show()
    # plt.close()
