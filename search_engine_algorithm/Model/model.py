...
from Utils._aux import create_dir, load_transformation, save_latent_space
...
class CustomVAE(VAE):
    ...
    def test_step(self, batch, batch_idx):
        ...
        x, y, fnames, ids,coord_x,coord_y = batch
        ...
        for i, (fname, coord_x, coord_y) in enumerate(zip(fnames, coord_x.tolist(), coord_y.tolist())):
            name = basename(fname.split('.tif')[0])
            save_latent_space(z[i], join(latent_spaces_dir, f"pred_{name}_({int(coord_x)},{int(coord_y)}).data"))
